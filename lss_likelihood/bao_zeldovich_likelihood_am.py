...
class ZeldovichBAOLikelihood(Likelihood):
    ...
    def compute_bao_pkmu(self, mu_obs, bao_sample_name):
        ...
        if self.recon_mode == 'prerecon':
            ...
            dampfac = np.exp( -0.5 * klin**2 * sigma_bao * rsd_fac )
    ...
    def bao_predict(self, bao_sample_name, thetas=None):
        ...
        if thetas is None:
            M0, M1, M2, M3, M4 = [self.linear_param_means[param_name + '_' + bao_sample_name] for param_name in ['M0','M1','M2','M3','M4']]
            Q0, Q1, Q2, Q3, Q4 = [self.linear_param_means[param_name + '_' + bao_sample_name] for param_name in ['Q0','Q1','Q2','Q3','Q4']]
        else:
            M0, M1, M2, M3, M4 = [thetas[param_name + '_' + bao_sample_name] for param_name in ['M0','M1','M2','M3','M4']]
            Q0, Q1, Q2, Q3, Q4 = [thetas[param_name + '_' + bao_sample_name] for param_name in ['Q0','Q1','Q2','Q3','Q4']]
        ...
        if self.fourier:
            p0 += polyval(klin/0.1,[M0,M1,M2,M3,M4]) / (klin/0.1)**2
            p2 += polyval(klin/0.1,[Q0,Q1,Q2,Q3,Q4]) / (klin/0.1)**2
